...
public class BlockBurner extends BlockContainer {
	...
	public BlockBurner(Material m) {
		super(m);
		this.setCreativeTab(Main.tabIndustry);
		this.setHardness(2.0F);
		this.setBlockName("burner");
		this.setBlockTextureName("burner");
		this.setStepSound(soundTypeMetal);
		this.setHarvestLevel("pickaxe", 3);
	}
	
	public Item getItemDropped(int i, Random r, int j) {
		return Item.getItemFromBlock(ModBlocks.burner);
	}

	@Override
	public TileEntity createNewTileEntity(World w, int meta) {
		return new TileEntityBurner(x, y, z);
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer p, int par6, float par7, float par8, float par9) {
		if(!(world.isRemote)) {
			p.openGui(Main.instance, Guis.GUI_BURNER, world, x, y, z);
			return true;
		}
		return false;
	}
    ...
    public void onBlockPlacedBy(World w, int x, int y, int z, EntityLivingBase e, ItemStack s) {
    	if(!w.isRemote) {
    		if(e instanceof EntityPlayer) {
    			...
    			p.addChatMessage(new ChatComponentText("Burner mis en place"));
    		}
    	}
		this.x = x;
		this.y = y;
